...
def test_products_count_by_category(product_factory, category_factory):
    ...
    for i in range(len(counts)):
        products = product_factory.create_batch(counts[i], category=categories[i])
...
def test_filter_customers_by_country(country_factory, customer_factory):
    ...
    customers2 = customer_factory.create_batch(10, country=country2)
    result = Customer.query.filter_country(country1).order_by(Customer.id).all()
    assert customers1 == result

def test_customer_email_unique(customer_factory):
    """ Test customer email must be unique"""
    try:
        customers = customer_factory.create_batch(2, email='customer@example.com')
...
def test_units_delivered_by_product_by_country(order_factory, product_factory, country_factory, customer_factory):
    """ Count how many units of each product where delivered grouping by country """
