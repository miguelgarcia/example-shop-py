...
def test_get(client, customer_factory):
    ...
    expected = model_to_dict(
        customer2,
        ['id', 'email', 'firstname', 'lastname', ('country', ['id', 'name'])])
...
def test_list_limit_offset(client, customer_factory):
    ...
    assert rv.headers['x-next'] == 'http://localhost/api/customers?offset=4\
&limit=3'
...
def test_customer_post(client, country_factory, db_session):
    ...
    rv = client.post('/api/customers', data=json.dumps(req),
...
def test_customer_update(client, customer_factory, country_factory):
    ...
    rv = client.put('/api/customers/{:d}'.format(customer.id),
    ...
    assert model_to_dict(
        customer,
        ['email', 'firstname', 'lastname', ('country', ['id'])]) == expect
...
def test_customer_update_404(client, customer_factory):
    ...
    rv = client.put('/api/customers/{:d}'.format(customer.id+1),
                    data=json.dumps(req), content_type='application/json')
    assert rv.status_code == 404
    data = json.loads(rv.data)
    assert expected_404 == data
